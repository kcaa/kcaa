...
class Variable(jsonobject.JSONSerializableObject):
    ...
    """Current value."""
    maximum = jsonobject.ReadonlyJSONProperty('max', value_type=int)
    """Maximum value."""


class RebuildingMaterial(jsonobject.JSONSerializableObject):
...
class ShipDefinition(jsonobject.JSONSerializableObject):
    ...
    capacity = jsonobject.ReadonlyJSONProperty(
        'capacity', value_type=resource.Resource)
    """Resource capacity."""
    ...
    """Cruising speed."""
    speed_class = jsonobject.ReadonlyJSONProperty('speed_class',
                                                  value_type=int)
    """Cruising speed class."""
    SPEED_CLASS_SLOW = 0
    SPEED_CLASS_FAST = 1
    ...
    rebuilding_material = jsonobject.ReadonlyJSONProperty(
        'rebuilding_material', value_type=RebuildingMaterial)
    """Rebuilding material."""
...
class ShipDefinitionList(model.KCAAObject):
    ...
    def update(self, api_name, response):
        super(ShipDefinitionList, self).update(api_name, response)
        for data in response['api_data']:
            ship_data = jsonobject.parse(data)
            # This KCSAPI returns empty results for unknown ships. Those
            # entries have a fuel capacity of 0.
            ...
            self.ships[ship_data.api_id] = ShipDefinition(
                ...
                rebuilding_material=RebuildingMaterial(
            ...
            #   api_enqflg, api_gumax, api_houk, api_houm, api_raik,
            #   api_raim, api_sakb, api_systems, api_touchs
            # Suspicious fields:
            ...
            #   api_getmes, api_gomes, api_gomes2, api_homemes, api_sinfo,
            #   api_voicef, api_yomi
